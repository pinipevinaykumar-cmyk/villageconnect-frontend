...
const SERVICE_KEYWORDS = ['service', 'electric', 'plumb', 'mechanic', 'repair', 'clean', 'carpenter', 'paint'];
const SUB_CATS = ['All', 'Electricians', 'Plumbers', 'Mechanics', 'Cleaning', 'Repairs'];
...
const ServicesPage = () => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const serviceCats = cats.filter(c =>
          SERVICE_KEYWORDS.some(kw => c.name.toLowerCase().includes(kw))
        );
        let allShops = [];
        if (serviceCats.length > 0) {
          const results = await Promise.all(
            serviceCats.map(c => API.get('/public/shops', { params: { categoryId: c.id } }))
          );
  ...
  const subFilter = (shop) => {
    ...
    if (activeSub === 'Electricians') return name.includes('electric');
    if (activeSub === 'Plumbers') return name.includes('plumb');
    if (activeSub === 'Mechanics') return name.includes('mechanic');
    if (activeSub === 'Cleaning') return name.includes('clean');
    if (activeSub === 'Repairs') return name.includes('repair');
    return true;
  };
    ...
    <div style={{ background: 'var(--bg)', minHeight: '100vh', paddingBottom: 90 }}>
      ...
      <div style={{ background: 'linear-gradient(160deg, #1E7B3B 0%, #2F855A 100%)', padding: '52px 18px 20px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
          ...
          <div>
            <div style={{ fontSize: 18, fontWeight: 800, color: 'white' }}>Local Services</div>
            <div style={{ fontSize: 12, color: 'rgba(255,255,255,.65)' }}>Electricians · Plumbers · Mechanics · Cleaning</div>
          </div>
        </div>
      ...
      <div style={{ padding: '14px 16px 0' }}>
        {loading ? (
          ...
          <div style={{ textAlign: 'center', padding: '60px 24px', background: 'var(--card)', borderRadius: 16, border: '1px solid var(--border)' }}>
            ...
            <div style={{ fontSize: 16, fontWeight: 700, color: 'var(--text)' }}>No services listed yet</div>
